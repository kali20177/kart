...
import { emitViewTabReactivate } from '@/utils/dockview-events'
 ...
 *  让面板内容组件把输入焦点拉回来（终端 xterm / 行输入条）。
 *  关闭按钮 @click.stop 已阻断冒泡，点 × 不会触发本广播。 */
function onTabClick() {
  if (isActive.value) emitViewTabReactivate(api)
}
