...
9. ✅ **统计面板太薄** — 已完成：`StatusBar` 新增帧数（RX/TX）、帧速率（f/s）、字节速率（B/s）、会话时长（HH:MM:SS）、缓冲使用率（百分比+>80%告警色）。速率每 1s 采样计算。连接断开自动归零。
...
20. ✅ **全局快捷键缺失** — 已有快捷键（`Ctrl/Cmd+Enter` 发送、`Alt/Ctrl+↑↓` 翻历史）已在帮助菜单新增「快捷键」面板展示，不额外添加显式 UI 已有操作的快捷键。
