...
### ⚠️ 二进制波形 X 轴时间与消息时间戳在 sampleRate 不匹配时漂移（待解决）

- **现象**：二进制解析模式下，波形 X 轴用**合成时间** `startTime + sampleIndex × (1000/sampleRate)`（假设传感器按 `sampleRate` 均匀采样），而消息时间戳是**字节到达时间**（`Date.now()`）。当 `sampleRate` 与传感器实际采样率不一致时，合成时间相对真实到达时间**线性漂移**，二者偏离越来越大。
- **验证**：`waveform.spec.ts` 的「二进制模式 X 时间对齐」用例--
  - `sampleRate` 匹配（640Hz、32 样本/50ms 帧）：每帧首采样 X = 到达时间，无漂移 ✓
  - `sampleRate` 配错（配 1000Hz、实际 640Hz）：帧1 合成 X=1032 vs 到达 1050（落后 18ms），帧2 合成 X=1064 vs 到达 1100（落后 36ms），线性增长 ✗
- **与文本模式的区别（为何文本模式已修而二进制未修）**：文本模式 1 行 = 1 采样，到达时间即采样时间无歧义，故已改用 `Date.now()` 作 X（见 `waveform.ts` ingest 文本分支 + `messages.ts` 帧时间戳改到达时间）。二进制模式 1 chunk = N 个采样，**无法从字节流恢复每个采样的真实时刻**--只能靠 `sampleRate` 假设帧内均匀分布。若也改用到达时间，N 个采样会塌缩到同一毫秒，帧内分辨率丢失、时间轴失效。
- **为何不照搬「真实到达时间」修法**：
  1. 多采样/帧塌缩：32 个采样全压到 chunk 到达那一毫秒，波形退化为重合点。
  2. 高速小帧流场景错误：若高采样率流（如 640Hz）以小帧（1~2 采样/帧、受 rAF 节流 ~16ms/帧）到达，按到达时间会算成 ~62.5Hz 间距，与真实采样率严重不符；合成时间反而是对的。
- **候选方案（未采用，代价明确）**：混合模型--每帧首采样锚定真实到达时间、帧内按 `dt` 均匀插值、相对上一采样保单调。可消除漂移并对齐消息时间戳，但：① 帧间会出现缺口（突发/小帧流），破坏严格均匀性（影响离线 FFT 等分析）；② `sampleRate` 变更无法再整体重算历史 X（锚点为真实时间），失去现有「改采样率即修正整段历史」能力；③ 对高速小帧流仍错误。综合代价大于收益，暂不实施。
- **当前处置**：作为已知限制保留。合成时间对「均匀采样的二进制传感器」（二进制模式的预期用途）是正确的，漂移仅在 `sampleRate` 配错时发生，属配置校准问题。用户将 `sampleRate` 配成与传感器实际采样率一致即可。若未来出现「采样率可变/未知」的真实场景，再评估混合模型。
- **相关代码**：`src/stores/waveform.ts`（ingest 二进制分支、sampleRate watch 重算逻辑）、`src/stores/messages.ts`（ingestRx 帧时间戳）、`docs/waveform-design.md` 第 9.3 节。
