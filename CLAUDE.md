...
- **端口占用标记**：枚举时主进程对每端口尝试独占打开探测（`dtr:false/rts:false`、成功立即关闭、1s 超时兜底），打开失败判定为被其他程序占用 → 下拉禁用 + 红色「被其他程序占用」，与会话间「已被其他会话占用」区分；`refreshPorts` 自动选中跳过 busy 端口。busy 为枚举时瞬时快照，需刷新端口重新探测；Web Serial 无此能力。
